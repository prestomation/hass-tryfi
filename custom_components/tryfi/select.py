...
async def async_setup_entry(
    ...
    entities = [TryFiLostModeSelect(coordinator, pet) for pet in coordinator.data.pets]
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        try:
            await self.hass.async_add_executor_job(
                self.pet.setLostDogMode, self.coordinator.data.session, is_lost
            )
        except Exception:
        ...
        _LOGGER.info("Set %s to %s mode", self.pet.name, option)
