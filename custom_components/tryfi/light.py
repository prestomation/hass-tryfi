...
def find_closest_color_code(
    target_color: tuple[int, int, int], color_map: dict[int, tuple[int, int, int]]
) -> int:
...
class TryFiPetLight(CoordinatorEntity, LightEntity):
    ...
    def __init__(self, coordinator: TryFiDataUpdateCoordinator, pet: FiPet) -> None:
        ...
        if (
            hasattr(pet.device, "availableLedColors")
            and pet.device.availableLedColors is not None
        ):
            for led_color in pet.device.availableLedColors:
                if hasattr(led_color, "ledColorCode") and hasattr(led_color, "hexCode"):
                    self._color_map[led_color.ledColorCode] = hex_to_rgb(
                        led_color.hexCode
                    )
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        await self.hass.async_add_executor_job(
            self.pet.turnOnOffLed, self.coordinator.data.session, True
        )

        # Set color if requested
        if ATTR_RGB_COLOR in kwargs:
            requested_color = kwargs[ATTR_RGB_COLOR]
            closest_color_code = find_closest_color_code(
                requested_color, self._color_map
            )
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        await self.hass.async_add_executor_job(
            self.pet.turnOnOffLed, self.coordinator.data.session, False
        )
